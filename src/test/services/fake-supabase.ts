...
export type DbState = {
  ...
  process_movement_notifications: Array<{ id: string; tenant_id: string; process_id: string; movement_id: string; contact_id: string; notified_at: string; status?: 'queued' | 'retry' | 'sent' | 'delivered' | 'failed' | 'dead_letter'; outbox_id?: string | null; last_error?: string | null; attempts?: number; updated_at?: string }>
